<template>
    ...
    <li v-bind:key="acct.accountId" v-for="acct in node.children" v-bind:class="{toggled: acct.toggled}">
        <p v-if="acct.assetBalance.length>0 && acct.assetBalance.filter(a => (Math.abs(a.value)>0.005)).length>0">
          <span
          ...
          >
            ...
            <span class="num other">
                    <span v-for="x in acct.assetBalance">{{x.value.toFixed(2)}} {{x.ccy}} </span>
            </span>
        </p>
