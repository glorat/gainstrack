<template>
  <div>
    <pre><span v-for="row in data"><template v-if="row.targetValue !== row.value">BUY {{ formatNumber(row.targetValue - row.value) }} {{ baseCcy }} of {{ row.assetId }}<template v-if="row.price!==1"> ({{ formatNumber((row.targetValue - row.value)/row.price) }} {{row.assetId}} @{{ row.price }} {{baseCcy}})</template></template>
</span></pre>
