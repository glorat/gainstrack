...
class BalanceTreeTable(acctState:AccountState, priceState:PriceState, date:LocalDate, balanceReport:BalanceReport, conversionStrategy:String) {
  val allAcctState = acctState.withInterpolatedAccounts


  def toTreeTable(acctId: AccountId):TreeTableDTO = {
    val balance = balanceReport.getState.convertedPosition(acctId, acctState, priceState, date, conversionStrategy)
