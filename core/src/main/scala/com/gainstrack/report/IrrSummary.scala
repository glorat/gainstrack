...
case class IrrSummary(accounts: Map[AccountId, AccountInvestmentReport]) {
  def toSummaryDTO: Seq[IrrSummaryItemDTO] = {
...
object IrrSummary {
  ...
  def apply(commands:Seq[AccountCommand], fromDate:LocalDate, queryDate:LocalDate, acctState: AccountState, balanceState: BalanceState, txState:TransactionState, singleFXConverter:SingleFXConverter) : IrrSummary = {
    ...
    val ret = invAccts.map(account => {
      ...
      val accountReport = new AccountInvestmentReport(accountId, ccy, fromDate, queryDate, acctState, balanceState, txState, singleFXConverter)
